-- FUNCTION: dbo.cordeliverytransaction_gettransactiondetails(date, date)

-- DROP FUNCTION IF EXISTS dbo.cordeliverytransaction_gettransactiondetails(date, date);

CREATE OR REPLACE FUNCTION dbo.cordeliverytransaction_gettransactiondetails(
  fromdate date,
  todate date)
    RETURNS TABLE(coredeliverytransactiondetailid bigint, fromentitycode text, fromentityid integer, fromentityname text, toentitycode text, toentityid integer, toentityname text, amount double precision, comission double precision, charges double precision, notes text, deliveryemployeename text, acceptedbycustomer boolean, acceptedbyemployee boolean, addedemployeename text, addedon timestamp with time zone) 
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION dbo.cordeliverytransaction_gettransactiondetails(date, date)
